...
const getTime = () => `${faker.random.number(10)} ${faker.random.arrayElement(['minutes', 'hours', 'seconds'])} ago`;
...
const userOwnMessages = Array.from({ length: COUNT_OF_OWN_MESSAGES }).map((_, i) => ({
  id: faker.random.uuid(),
  color: ownColor,
  time: getTime(),
  username: ownName,
...
const userStatuses = Array.from({ length: COUNT_OF_STATUSES }).map((_, i) => ({
  id: faker.random.uuid(),
  color: getColor(),
  time: getTime(),
  username: getName(),
